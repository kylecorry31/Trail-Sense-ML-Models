...
#
# Modifications made by Kyle Corry to use the model for cloud classification (and removed the CLI)
# ==============================================================================
...
_MODEL_INFO = {
    "model_quantized.tflite":
        ModelSpecificInfo(
            name="Trail Sense cloud image classifier",
            version="v1",
            image_width=48,
            image_height=48,
            image_min=0,
...
class MetadataPopulatorForImageClassifier(object):
  ...
  def _create_metadata(self):
    ...
    model_meta.description = ("Identify the cloud genus")
    model_meta.version = self.model_info.version
    model_meta.author = self.model_info.author
    model_meta.license = ("MIT"
                          "https://github.com/kylecorry31/Trail-Sense/blob/main/LICENSE")
...
def main():
  model_file = "model_quantized.tflite"
  model_basename = os.path.basename(model_file)
  if model_basename not in _MODEL_INFO:
    raise ValueError(
        "The model info for, {0}, is not defined yet.".format(model_basename))
  
  os.mkdir("model_with_metadata")

  export_model_path = os.path.join("model_with_metadata", model_basename)
  ...
  populator = MetadataPopulatorForImageClassifier(
      export_model_path, _MODEL_INFO.get(model_basename), "labels.txt")
  populator.populate()

  # Validate the output model file by reading the metadata and produce
  # a json file with the metadata under the export path
  displayer = _metadata.MetadataDisplayer.with_model_file(export_model_path)
  export_json_file = os.path.join("model_with_metadata",
